...
- 服务器资产支持搜索、验证后删除和同地址重新接入；删除采用保留历史关系的逻辑删除。
- 保存服务器前检查 SSH、sudo、Python 3、操作系统、x86_64 架构和磁盘空间。
- 通用软件包仓库：支持各中间件的源码包和已编译二进制包。
- 软件包仓库支持搜索和受控删除；仅删除仓库目录中经过校验的目标单文件。
...
- 已完成或失败任务支持从任务列表删除，部署日志和操作审计仍保留。
- Redis 部署成功后自动生成 Word 交付报告，包含部署拓扑、服务器、端口、目录与访问密码，支持重复下载。
...
- Word 交付报告包含 Redis 明文密码，只允许管理员和操作员下载，每次下载均写入审计日志。
